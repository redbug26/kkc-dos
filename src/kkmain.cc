...
void MouseShortCut(void) {
    ...
    if (DFen->F[DFen->pcur]->name[0] == '*') {
        nbr = 1;
        bar[0].Titre = "Run Function";
        bar[0].fct = 84;
    } else if ((DFen->F[DFen->pcur]->attrib & RB_SUBDIR) == RB_SUBDIR) {
        nbr = 6;
        bar[0].Titre = "Enter";
        bar[0].fct = 84;
        bar[1].Titre = NULL;
...
void GetBufKey(int key, char* buffer) {
    ...
    switch (key) {
        ...
        case KEY_F(1) + 0x1900:
            strcpy(buffer, "Shift-F1");
            break;
        case KEY_F(2) + 0x1900:
            strcpy(buffer, "Shift-F2");
            break;
        case KEY_F(3) + 0x1900:
            strcpy(buffer, "Shift-F3");
            break;
        case KEY_F(4) + 0x1900:
            strcpy(buffer, "Shift-F4");
            break;
        case KEY_F(5) + 0x1900:
            strcpy(buffer, "Shift-F5");
            break;
        case KEY_F(6) + 0x1900:
            strcpy(buffer, "Shift-F6");
            break;
        case KEY_F(7) + 0x1900:
            strcpy(buffer, "Shift-F7");
            break;
        case KEY_F(8) + 0x1900:
            strcpy(buffer, "Shift-F8");
            break;
        case KEY_F(9) + 0x1900:
            strcpy(buffer, "Shift-F9");
            break;
        case KEY_F(10) + 0x1900:
            strcpy(buffer, "Shift-F10");
            break;

        case KEY_F(1) + 0x2300:
            strcpy(buffer, "Ctrl-F1");
            break;
        case KEY_F(2) + 0x2300:
            strcpy(buffer, "Ctrl-F2");
            break;
        case KEY_F(3) + 0x2300:
            strcpy(buffer, "Ctrl-F3");
            break;
        case KEY_F(4) + 0x2300:
            strcpy(buffer, "Ctrl-F4");
            break;
        case KEY_F(5) + 0x2300:
            strcpy(buffer, "Ctrl-F5");
            break;
        case KEY_F(6) + 0x2300:
            strcpy(buffer, "Ctrl-F6");
            break;
        case KEY_F(7) + 0x2300:
            strcpy(buffer, "Ctrl-F7");
            break;
        case KEY_F(8) + 0x2300:
            strcpy(buffer, "Ctrl-F8");
            break;
        case KEY_F(9) + 0x2300:
            strcpy(buffer, "Ctrl-F9");
            break;
        case KEY_F(10) + 0x2300:
            strcpy(buffer, "Ctrl-F10");
            break;

        case KEY_F(1) + 0x2D00:
            strcpy(buffer, "Alt-F1");
            break;
        case KEY_F(2) + 0x2D00:
            strcpy(buffer, "Alt-F2");
            break;
        case KEY_F(3) + 0x2D00:
            strcpy(buffer, "Alt-F3");
            break;
        case KEY_F(4) + 0x2D00:
            strcpy(buffer, "Alt-F4");
            break;
        case KEY_F(5) + 0x2D00:
            strcpy(buffer, "Alt-F5");
            break;
        case KEY_F(6) + 0x2D00:
            strcpy(buffer, "Alt-F6");
            break;
        case KEY_F(7) + 0x2D00:
            strcpy(buffer, "Alt-F7");
            break;
        case KEY_F(8) + 0x2D00:
            strcpy(buffer, "Alt-F8");
            break;
        case KEY_F(9) + 0x2D00:
            strcpy(buffer, "Alt-F9");
            break;
        case KEY_F(10) + 0x2D00:
            strcpy(buffer, "Alt-F10");
            break;
        ...
        case KEY_F12:
            ...
            break;
        case KEY_F11 + 0x0200:
            strcpy(buffer, "Shift-F11");
            break;
        case KEY_F12 + 0x0200:
            strcpy(buffer, "Shift-F12");
            break;
        case KEY_F11 + 0x0400:
            strcpy(buffer, "Ctrl-F11");
            break;
        case KEY_F12 + 0x0400:
            strcpy(buffer, "Ctrl-F12");
            break;
        case KEY_F11 + 0x0600:
            strcpy(buffer, "Alt-F11");
            break;
        case KEY_F12 + 0x0600:
            strcpy(buffer, "Alt-F12");
            break;
        ...
        case KEY_INS:
            ...
            break;
        case KEY_A_ENTER:
            strcpy(buffer, "Alt-ENTER");
            break;

        case KEY_C_PPAGE:
            strcpy(buffer, "Ctrl-PGUP");
            break;
        case KEY_C_NPAGE:
            strcpy(buffer, "Ctrl-PGDN");
            break;
        case KEY_C_UP:
            strcpy(buffer, "Ctrl-UP");
            break;
        case KEY_C_DOWN:
            strcpy(buffer, "Ctrl-DOWN");
            break;
        case KEY_C_RIGHT:
            strcpy(buffer, "Ctrl-RIGHT");
            break;
        case KEY_C_LEFT:
            strcpy(buffer, "Ctrl-LEFT");
            break;
...
char* AccessFile(int n) {
    ...
    switch (DFen->system) {
        ...
        case 9:                 // DKF
            ...
            DFen->nopcur = n;
            Copie(DFen, Fenetre[2]->path);  // Quitte apr�s
            ...
            while ((!KbHit()) & (c == 0))  // RedBug
            {
                ...
                if (DFen->sizedir == 1)
                    for (n = 0; n < DFen->nbrfic; n++) {
                        ...
                        if ((F->info == NULL) & ((F->attrib & RB_SUBDIR) == RB_SUBDIR) & (F->name[0] != '.')) {
                    ...
                    if ((xm >= DFen->Fen2->x) & (xm <= DFen->Fen2->xl) &
                        ...
                        if ((ym < DFen->yl) & (ym >= DFen->y2) & (xm == DFen->xl) &
                            ...
                            if ((ym <= DFen->y2) & (ym >= DFen->y) & (xm >= DFen->x) &
                                ...
                                oldzm = 0;
                                c = 3;
                            } else if ((ym > DFen->yl2 + DFen->y2) & (ym <= DFen->yl) &
                                       (xm >= DFen->x) & (xm <= DFen->xl)) {
                                GestionFct(36);
                                oldzm = 0;
                                c = 3;
                            } else if ((iskeypressed(SHIFT)) & (xm >= DFen->x) & (xm <= DFen->xl) &
                                       (ym <= DFen->yl2 + DFen->y2) & (ym > DFen->y2))  //- Shift -
                            {
                                ...
                                if (oldsel == -1) {
                                    if ((ym == Cfg->TailleY - 1) & (KKCfg->isbar))
                                        GetMouseFctBar(0);
                                    else if (((xm < DFen->Fen2->x) | (xm > DFen->Fen2->xl)) &
                                             ((xm < DFen->x) | (xm > DFen->xl)) &
                                    ...
                                    else if ((ym == Cfg->TailleY - 1) & (KKCfg->isbar))  //- Fx key -
                                    {
                                        c = GetMouseFctBar(2);
...

                                    {
                                        ...
                                        if ((ym < DFen->yl - 3)  //--- Barre du milieu ---------
                                            & ((xm < DFen->x) | (xm > DFen->xl)) & ((xm < DFen->Fen2->x) | (xm > DFen->Fen2->xl))) {
                                            n = (ym - 1) / 3;
                                            if ((n >= 0) & (n < 6)) {
                ...
                if (iskeypressed(SHIFT))
                    MenuBar(1);
                else if (iskeypressed(ALT))
                    MenuBar(2);
                else if (iskeypressed(CTRL))
                    MenuBar(3);
                else
                    MenuBar(0);
            }
...
int main(int argc, char** argv) {
    ...
    */
    ...
    CreateNewFen(Fenetre[2], 0);  // Fen�tre ferm�e par default
    Fenetre[2]->x = 40;
    Fenetre[2]->nfen = 2;
...
/*--------------------------------------------------------------------*\
|- Gestion de la fen�tre ext�rieure 								  -|
\*--------------------------------------------------------------------*/
