...
Bar(" Help  ----  Info  ----  ----  O.S.  CRC- Fr_end ----  Quit ");
...
switch(car)
    ...
    case KEY_F(9):      // F9  (KKT)
        ...
        if (Seekfile(10,10,buf)==0)
            ...
            FILE *fic;
            fic=fopen(buf,"rb");
            lenbox=(short)fread(box,1,16384,fic);
            fclose(fic);
            }
        break;
    case KEY_F(7):      // F7  (CRC-32)
        strcpy(buf,Filename);
        n=0;
        while(buf[n]!=0)
        ...
        break;
    case KEY_F(6):      // F6  (Operating system)
        KKos=(KKos==0) ? 1 : 0;
        break;
