...
extern FENETRE *Fenetre[NBWIN]; // uniquement pour trouver la 3‚me trash
...
if ((F->longname==NULL) &
    ...
    (F->name[0]!='*'))
	{
    F->longname=(char*)GetMem(strlen(chaine)+1);
    memcpy(F->longname,chaine,strlen(chaine)+1);
	}
...
if (*Info->fullname!=0)
    ...
    strcpymax(buffer,Info->fullname,31);
...
if (*Info->format!=0)
	{
    strcpymax(buffer,Info->format,31);
...
if (*Info->composer!=0)
	{
    strcpymax(buffer,Info->composer,24);
...
if (affmsg)
	...
	for (m=0;m<9;m++)
		...
		if (*Info->message[m]!=0)
			{
            strcpymax(buffer,Info->message[m],36);
            sprintf(ligne,"%-36s\r\n",buffer);
			strcat(Buf,ligne);
			}
		}
...
switch(c)
	...
	case 3:
		AltMenu();
//		  d=-1;  //--- Pour mettre … jour (pour la date)
		break;
	default:
		c=4;
		break;
...
F=GetFile(Fen->Fen2,-1);

strcpy(path,Fen->Fen2->path);
if (F->name[0]!='.')
    Path2Abs(path,F->name);  // Ajout gedeon
...
switch (Cfg->windesign)
	...
	case 1:
		...
		PrintAt(x,y,	  "ÉÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ»");
		PrintAt(x,y+1,	  "º                                      º");
		PrintAt(x,yl-sb-1,"ÇÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¶");
	...
	case 2:
		...
		PrintAt(x,y,	  "ÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛÛ");
		...
		break;
	case 3:
		PrintAt(x,y,	  "ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿");
		PrintAt(x,yl-sb-1,"³ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿³");
	...
	if ( (n==Fen->oldpcur) | (n==Fen->pcur) | (dispall) )
		...
		while(disp[m]!=0)
			...
			switch(disp[m])
				...
				case 5:
                    strcpy(nom,F2->name);
                    if ((F2->attrib & RB_SUBDIR)!=RB_SUBDIR)
						strlwr(nom);
                    ...
                    if ((F2->attrib & RB_HIDDEN)==RB_HIDDEN)
						ch1[8]=176;  // 176,177,178

					if ( (Fen->system==5) &
                         (F2->desc==1) &
                         ((F2->attrib & RB_SUBDIR)!=RB_SUBDIR) )
						ch1[8]=0x18;

                    if (F2->longname!=NULL)
						{
                        strcpy(buf,F2->longname);
						buf[18]=0;			//--- 12
						sprintf(ch5,"%-18s",buf);
						}
						else
					...
					break;
				case 2:
                    sprintf(ch2,"%10s",Long2Str(F2->size,temp2));
                    if ( ((F2->attrib & RB_SUBDIR)==RB_SUBDIR) &
                         (F2->size==0) )
						strcpy(ch2,_LabelSubDir);

                    if ((F2->attrib & RB_VOLID)==RB_VOLID)
...
strcpy(Buf,"                                    \r\n");
strcat(Buf,"        IDF File Information        \r\n");
...
// ------------------ Line Activity ------------------------------------
    ...
    if (F->info!=NULL)
		{
        memcpy(buffer,F->info+1+Fen2->InfoPos,38);
		buffer[38]=0;
...
if ((KKCfg->longname) & (DFen->system==0))
    ...
    strcpy(_intbuffer,GetFilename(DFen,-1));
...
PrintAt(x,posy," %s",RBTitle);     ColLin(x,posy,38,Cfg->col[40]);  posy++;
ChrLin(x,posy,38,196);	posy++;

PrintAt(x,posy," Current directory");
ColLin(x,posy,38,Cfg->col[40]); posy++;
PrintAt(x,posy," %s",Buf); posy++;
ChrLin(x,posy,38,196); posy++;

PrintAt(x,posy," Current disk");
ColLin(x,posy,38,Cfg->col[40]);  posy++;

PrintAt(x,posy," %s [%s] %s",disk,volume,InfoVol); posy++;

PrintAt(x,posy," Free space: %8s kilobytes",Long2Str(tfree,temp));  posy++;
PrintAt(x,posy," Capacity:   %8s kilobytes",Long2Str(ttotal,temp)); posy++;

ChrLin(x,posy ,38,196); posy++;
PrintAt(x+1,posy,"System Information");
ColLin(x+1,posy,37,Cfg->col[40]);	posy++;

PrintAt(x,posy," þ %s",chaine);   posy++;   // Information about windows
PrintAt(x,posy," þ Initialisation: %d clocks ",Info->temps);   posy++;
PrintAt(x,posy," þ Long filenames support: %s ",
									  KKCfg->longname ?"Yes": "No"); posy++;
...
PrintAt(x,posy," þ Date & Time: %21s",timebuf); posy++;
