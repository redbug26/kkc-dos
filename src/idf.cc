...
static char *FORMCMP=" Compressed with%19s";
static char *FORMUSE=" Using%29s";
static char *FORMCOM=" Compiled with%21s";
static char *FORMCRY=" Crypted by%24s";
static char *FORMLNK=" Link to%27s";
static char *FORMSID=" Microsoft ID:                 %04X";
...
if (!memcmp(buf,"8CHN",4)) { strcpy(Info->format,"PC-FTracker");
                                                     chnl=8; instr=31; }
if (!memcmp(buf,"10CH",4)) { strcpy(Info->format,"PC-FTracker");
                                                    chnl=10; instr=31; }
...
sprintf(Info->message[8]," Header is %18ld bytes",((int)header)*16);
