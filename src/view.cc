/*--------------------------------------------------------------------*\
|- Viewer:															  -|
|		   html: illimité (dépend de la mémoire)					  -|
...
class KKFile {
   public:
    ...
    char namebuf[256];  //--- Titre du fichier réduit ----------
    ...
    if (fic != NULL) {
        ...
        while (fread(&n, 1, 1, fic) == 1) {
            ...
            if (!stricmp(ficname, filename)) {
                pos = ftell(fic);
                fseek(fic, pos, SEEK_SET);  // Parce qu'on écrit juste après
            ...
            if (iskeypressed(SHIFT)) {
                ...
                if ((button & 1) == 1)  //--- gauche --------------------------------
                {
                    if ((x == 78 + x1) & (y >= 4) & (y <= Cfg->TailleY - 3))  //- Ascens.--
                    {
                        ...
                        car = 1;
                    } else if (y == Cfg->TailleY - 1)
                        if (Cfg->TailleX == 90)
                            car = (0x3B + (x / 9)) * 256;
                        else
                            car = (0x3B + (x / 8)) * 256;
                    else if (y > (Cfg->TailleY) / 2)
                        car = 80 * 256;
                    else
                        car = 72 * 256;
                }
...
void RefreshBar(char* bar) {
    ...
    {
        case 0:
            memcpy(bar + 6, "Nowrap", 6);
            break;
        case 1:
            memcpy(bar + 6, " Wrap ", 6);
            break;
        case 2:
            memcpy(bar + 6, "WoWrap", 6);
            break;
    }
}
...
int TxtView(KKFile& Fic) {
    ...
    if (ViewCfg->ajustview) {
        ...
        if (xm >= ((Cfg->TailleX) - 1))
            ...
            autowarp = 1, xl = xm;  //--> Longueur fenêtre
    ...
    strcpy(bar, " Help NowrapLnFeedChView Zoom PreproSearch Print Mask  Quit ");
    ...
    do {
        ...
        do {
            ...
            if ((tpos >= xl2) & (aff == 0))  // Si le prochain dépasse la fenêtre
            {
                if (tpos >= xl2) {
                    if ((ViewCfg->warp == 1) & (autowarp == 0)) {
                        aff = 2;

                        w1 = xl2;   // Premier à retenir
                        w2 = tpos;  // Dernier à retenir
                    }
...
\*--------------------------------------------------------------------*/
static char srcch[132];  //--- chaine à rechercher --------------
...
void Masque(int x1, int y1, int x2, int y2, char* bufscr, char* bufcol) {
    ...
    /*--------------------------------------------------------------------*\
    |- Affichage de la chaine qui a été trouvé (par F7) 				  -|
    \*--------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------*\
|- Changement du Line Feed pour le passage à la ligne				  -|
\*--------------------------------------------------------------------*/
int ChangeLnFeed(void) {
    MENU menu;
    int nbr;
...
int AnsiView(KKFile& Fic) {
    ...
    strcpy(bar, " Help Spart  ---- ChView ---- Prepro ----  ----  ----  Quit ");
...
void ChgViewPreproc(KKFile& Fic) {
    ...
    do {
        ...
        if (retour == -1)  //--- En haut ------------------------------------------
        {
            ...
            } else if (bar[m].fct != 0)  //--- Deplacement -------------------------------
            {
                char t1, t2;
                int pos;
...
void HtmLine::AddChar(char car) {
    ...
    if (smot > xpage)  //--- le paragraphe dépasse une ligne ------------------
    {
        char cesure;
    ...
    //--- Quelqu'un a demandé un passage à la ligne --------------------
...
int HtmlView(KKFile& Fic, int& debpos) {
    ...
    int derspace;  //--- Vaut 1 si le dernier caractere était un space ----
    ...
    Fic.SavePosition(ViewCfg->viewhist, 1);  //- Pour l'historique après -
            ...
            switch (car) {
                case '>':
                    ...
                    if (debut < 256) {
                            ...
                            if ((src != NULL) & (alt == NULL)) {
                                ...
                                if (src != NULL) {
                                    ...
                                    if (!strcmp(src, "INTERNAL-GOPHER-IMAGE"))
                                        strcpy(titre, "\03");
                                    else if (!strcmp(src, "INTERNAL-GOPHER-UNKNOWN"))
                                        strcpy(titre, "?");
                                    else if (!strcmp(src, "INTERNAL-GOPHER-TEXT"))
                                        strcpy(titre, "#");
                                    else if (!strcmp(src, "INTERNAL-GOPHER-SOUND"))
                                        strcpy(titre, "\0E");
                                    else
                                        titre[0] = 0;
                                }
                        ...
                        if (!strcmp(tag, "CENTER"))  //--- Paragraphe centré --------
                        {
                            Line.Aff();
        ...
        if (code != 0)  // code &...;
        {
            ...
            switch (car) {
                ...
                default:
                    ...
                    if (!strnicmp(titre, "EGRAVE", 6)) psuiv = 1, car = 'è';
                    if (!strnicmp(titre, "EACUTE", 6)) psuiv = 1, car = 'é';

                    if (!strnicmp(titre, "AGRAVE", 6)) psuiv = 1, car = 'à';
                    if (!strnicmp(titre, "AACUTE", 6)) psuiv = 1, car = 'á';
                    ...
                    if (!strnicmp(titre, "CCEDIL", 6)) psuiv = 1, car = 'ç';

                    if (!strnicmp(titre, "ECIRC", 5)) psuiv = 1, car = 'ê';
                    if (!strnicmp(titre, "ACIRC", 5)) psuiv = 1, car = 'â';
                    if (!strnicmp(titre, "ICIRC", 5)) psuiv = 1, car = 'î';
                    if (!strnicmp(titre, "UCIRC", 5)) psuiv = 1, car = 'û';
                    if (!strnicmp(titre, "OCIRC", 5)) psuiv = 1, car = 'ô';
                    ...
                    if (psuiv == 1)  //--- La valeur est décodé correctement ----
                    {
                        code = 0;
                        /*				  if (lentit!=0)
                                                                GetPreprocString(titre,lentit);
    ...
    for (i = 0; i < yl - y - split; i++)  //--- Prépare la derniere ligne à l'écran --
    {
        if (DLine->next != NULL)
            DLine = DLine->next;
    }

    for (i = 0; i < ye0; i++)  //--- On ne commence pas à la premiere ligne ---
