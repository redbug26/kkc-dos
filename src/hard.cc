...
x=inp(0x3CC);
x=(x&0xF3)|4;						   // mets les bits 2-3 Ö 01 = 28MhZ
outp(0x3C2,(char)x);
...
regs.w.ax=0x1003;					// Donne la palette n Ö la couleur n
regs.w.bx=0;
int386(0x10,&regs,&regs);
...
signed long _WhichEcran=0;				 //--- Nbr d'Çcran en memoire --


long _EcranX[10],_EcranY[10];			 //--- Position du curseur -----
char _EcranD[10],_EcranF[10];			 //--- Definition du curseur ---
signed long _WhichState=0;				 //--- Nbr d'Çtats en memoire --
...
PrintAt(colonne,ligne,chaine); //RÇÇcrit la chaine Ö la position dÇsirÇe
...
switch (car&255)
	...
	case 9:
		...
		break;
	case 0: 			// vÇrifier si pas de touche de fonction pressÇe
		c2=(car/256);
		...
		switch (c2)
			...
			case 80:											 // DOWN
				...
				break;
			case 83:											  // del
				if (i!=fin) 		// vÇrifier si pas premiere position
					{
					fin--;
					*(chaine+fin+1)=' ';
					*(chaine+fin+2)='\0';
		...
		case 8: 							 // vÇrifier si touche [del]
			if (i>0)				// vÇrifier si pas premiere position
				{
				i--;
				fin--;
				if (i!=fin)
		...
		default:					   // vÇrifier si caractäre correcte
			if ((car>31) && (car<=255))
				{
				if ((i==fin) || (!ins))
					{
...
void *GetMemSZ(int s)						  // GetMem sans mise Ö zÇro
{
void *buf;
...
/*--------------------------------------------------------------------*\
|- Initialise les adresses dans la mÇmoire vidÇo					  -|
\*--------------------------------------------------------------------*/
void InitSeg(void)
{
int n;
...
int _Ansi_tcol; 				// Couleur que l'on a demandÇ par affcol
...
int _Com_tcol;					// Couleur que l'on a demandÇ par affcol
